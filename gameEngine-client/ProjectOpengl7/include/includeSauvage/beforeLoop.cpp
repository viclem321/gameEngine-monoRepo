...
OtherPlayers::model = new Model("ressources/perso/perso.fbx", glm::vec3(0.0f, 3.0f, 0.0f), glm::vec3(0.8f, 3.0, 0.6f));
OtherPlayers::allAnimations[0] = new Animation("ressources/perso/perso.fbx", OtherPlayers::model, 0);  // idle
OtherPlayers::allAnimations[1] = new Animation("ressources/perso/perso.fbx", OtherPlayers::model, 1);  // run
OtherPlayers* allPlayers[InfoGlobal::nMaxClients];
for(int i=0; i<InfoGlobal::nMaxClients; i++) { allPlayers[i] = new OtherPlayers(); }

//-----------INITIALISATION DES DONNES DU JEU-----------


//---------- CONFIG VAO VBO EBO UBO -------------

	//tampon variable uniform
	unsigned int UBO1;
	glGenBuffers(1, &UBO1);
...
//------------- CHARGEMENT DES ELEMENTS -----------------
	...
	//load shaders
	Shader shaderModelMat("fichierTexte/vertexShaderModelMat.txt", "fichierTexte/fragmentShaderModel.txt");
	Shader shaderModelVec("fichierTexte/vertexShaderModelVec.txt", "fichierTexte/fragmentShaderModel.txt");
	Shader shaderPerso("fichierTexte/vertexShaderPerso.txt", "fichierTexte/fragmentShaderPerso.txt");

	//load map
	Map map1("ressources/maps/heightMapTerrain4.png", "map1", 960, 130, "ressources/maps/terrain1.obj"); // ne pas oublier de remettre terrain1.obj
	//load character
	Character character("victor", "ressources/perso/perso.fbx", glm::vec3(840.0f, 30.0f, 440.0f));


	//create camera
	Camera camera(&character, glm::vec3(0.0f, 1.0f, 0.0f));




	Model ourModelTree1("ressources/mytree1.obj", glm::vec3(0.0f, 10.0f, 0.0f), glm::vec3(6.f, 10.f, 9.0f), 'N');
	...
	Model ourModelTree2("ressources/mytree2.obj", glm::vec3(0.0f, 21.0f, 0.0f), glm::vec3(24.f, 21.f, 20.0f), 'N');
	...
	Model ourModelTree3("ressources/mytree3.obj", glm::vec3(0.0f, 20.0f, 0.0f), glm::vec3(8.f, 20.f, 8.0f), 'N');
	...
	Model ourModelTree4("ressources/mytree4.obj", glm::vec3(0.0f, 12.0f, 0.0f), glm::vec3(6.f, 12.f, 6.0f), 'N');
	...
	Model ourModelTree5("ressources/mytree5.obj", glm::vec3(0.0f, 8.0f, 0.0f), glm::vec3(3.f, 8.f, 3.0f), 'N');
	...
	Model ourModelTree6("ressources/mytree6.obj", glm::vec3(0.0f, 4.5f, 0.0f), glm::vec3(2.f, 4.5f, 2.0f), 'N');
	...
	Model ourModelHouse1("ressources/myHouse1.obj", glm::vec3(0.0f, 43.0f, 0.0f), glm::vec3(36.f, 43.f, 64.0f), 'N');
	...
	Model ourModelRock1("ressources/rock1.obj", glm::vec3(0.0f, 11.056f, -1.02f), glm::vec3(13.45f, 13.f, 22.95f), 'N');
...
sockaddr_in addr; inet_pton(AF_INET, "192.168.1.201", &addr.sin_addr); addr.sin_port = htons(5001); addr.sin_family = AF_INET; InfoGlobal::myUdp = new Udp(2001, addr);
DatagramServerReceived datagramsReceived; datagramsReceived.allDatagrams = static_cast<DatagramByServer*>(calloc(static_cast<size_t>(SIZE_TAMPON_RECEIVED), sizeof(DatagramByServer))); datagramsReceived.pullEmplacement = 0; datagramsReceived.pushEmplacement = 0;
DatagramServerReceived datagramsReceivedWaited; datagramsReceivedWaited.allDatagrams = static_cast<DatagramByServer*>(calloc(static_cast<size_t>(SIZE_TAMPON_RECEIVED/2), sizeof(DatagramByServer))); datagramsReceivedWaited.pullEmplacement = 0; datagramsReceivedWaited.pushEmplacement = 0;

if(InfoGlobal::myUdp->processHandshake() == false)
{
	if(InfoGlobal::myUdp->processHandshake() == false)
	{
		std::cout << "Echec de la connexion au serveur. Reesayez plus tard " << std::endl;
		return 0;
	}
}
...
Sleep(20);
//find the good frame
auto start = std::chrono::steady_clock::now(); start -= InfoGlobal::frameTime;
if(EgalizeNetwork(start, datagramsReceived) == false) {std::cout << "Egalize Network fail" << std::endl; return 0;} 
std::cout << "egalizeNetwork succes" << std::endl;
if(RecuperateStateOfTheGame(start, character, allPlayers, map1, datagramsReceived) == false) {std::cout << "RecuperateStateOfTheGame fail" << std::endl; return 0;}
std::cout << "Recuperate state succes" << std::endl; 
